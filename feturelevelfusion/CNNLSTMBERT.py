...
directory_text = "C:/Users/44746/Desktop/Project/BERT-Reading"
...
def apply_pca(features, n_components=95):  
    pca = PCA(n_components=n_components)
    transformed_features = pca.fit_transform(features)
    return transformed_features


# Update load_features_for_fold to include PCA
def load_features_for_fold(directory_audio, directory_text, filenames, n_pca_components=100):
    segments = []
    labels = []
    identifiers = []
    ...
    for identifier in filenames:
        ...
        text_filename = f"{identifier}_rt_bert_features.csv"
        ...
        if os.path.exists(audio_path) and os.path.exists(text_path):
            ...
            features = apply_pca(features, n_pca_components)  # Apply PCA 
    ...
    return np.array(segments, dtype=np.float32), np.array(labels, dtype=np.int32), identifiers
...
for fold_idx in range(len(fold_files)):
    ...
    # When initializing your model
    model = CNNRNNModel(num_channels=100, hidden_dim=128, num_layers=2, bidirectional=False)  # Adjust num_channels to match PCA components
    ...
    # Training loop
    model.train()
    for epoch in range(5): # 10 for RT, 15 for IT yields best performance
